<template>
	<view class="editor-container">
		<view id="fixed-top" class="fixed-top" :class="{ isFixed: isFixed }" v-show="isfixedtop">
			<slot></slot>
			<button class="btn btn-primary" @click="save">保存</button>
		</view>
		<view class="fixed-top__place" v-show="isfixedtop"></view>
...
export default {
	...
	methods: {
		...
		formatformat(bind, item = {}, pitem = {}) {
			...
			switch (bind) {
				...
				case 'insertDate': //插入时间
					var date = new Date();
					var formatDate = `${date.getFullYear()} 年${date.getMonth() +
						1} 月${date.getDate()} 日`;
		...
		onSuccess(file, res) {
			...
			if (json.code == 0) {
				this.insertImage(resuest_config.baseURL + json.data.path, file);
			} else {
				uni.showToast({
					title: '图片上传失败',
					icon: 'none'
